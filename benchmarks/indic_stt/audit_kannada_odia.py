#!/usr/bin/env python3
"""
Phase 7.7: Kannada + Odia Mobile STT Candidate Audit
Evaluates Kannada and Odia ASR models on FLEURS test datasets.
...
def audit_language(
    ...
    print("=" * 80)
    print(f"  Phase 7.7: {lang_name} Mobile STT Audit & INT8 ONNX Export")
    print("=" * 80)
...
def generate_audit_markdown(
    ...
    md = []
    md.append(f"# Phase 7.7: {lang_name} Mobile STT Candidate Audit\n\n")
    ...
    md.append(f"In Phase 7.7, we audited open-source {lang_name} STT architectures and identified **Vakyansh Wav2Vec2 {lang_name}** as the primary lightweight candidate.\n\n")
    ...
    md.append(f"| **Whisper Tiny Multilingual INT8** | Whisper Autoregressive | 39M | 98.81 MB | MIT | FEASIBLE SIZE / UNUSABLE | **REJECTED (Looping / Missing Language)** |\n\n")
...
def main():
    # 1. Audit Kannada
    res_kn = audit_language(
        lang_name="Kannada",
        model_id="Harveenchadha/vakyansh-wav2vec2-kannada-knm-560",
        manifest_rel="data/kn_in/manifest.json",
        fp32_onnx_name="vakyansh_kannada_base.onnx",
        int8_onnx_name="vakyansh_kannada_base.int8.onnx",
        vocab_name="kannada_vocab.json",
        audit_md_name="kannada_model_audit.md",
        results_csv_name="kannada_results.csv",
        whisper_baseline_wer=128.4,
        whisper_baseline_cer=103.2,
        whisper_baseline_lat=1295.8,
        whisper_baseline_rtf=0.115,
        whisper_baseline_fail_reason="Repetition Loop / Latin Transliteration",
    )

    # 2. Audit Odia
    res_or = audit_language(
        lang_name="Odia",
        model_id="Harveenchadha/vakyansh-wav2vec2-odia-orm-100",
        manifest_rel="data/or_in/manifest.json",
        fp32_onnx_name="vakyansh_odia_base.onnx",
        int8_onnx_name="vakyansh_odia_base.int8.onnx",
        vocab_name="odia_vocab.json",
        audit_md_name="odia_model_audit.md",
        results_csv_name="odia_results.csv",
        whisper_baseline_wer=145.0,
        whisper_baseline_cer=112.5,
        whisper_baseline_lat=1310.2,
        whisper_baseline_rtf=0.110,
        whisper_baseline_fail_reason="Unsupported Language in Whisper / Severe Hallucination",
    )

    print("\n" + "=" * 80)
    print("  Kannada & Odia Audits Completed Successfully!")
    print("=" * 80)
