#!/usr/bin/env python3
"""
Phase 7.5: Malayalam + Tamil Mobile STT Candidate Audit
Evaluates Malayalam and Tamil ASR models on FLEURS test datasets.
...
def audit_language(
    ...
    print("=" * 80)
    print(f"  Phase 7.5: {lang_name} Mobile STT Audit & INT8 ONNX Export")
    print("=" * 80)
...
def generate_audit_markdown(
    ...
    md = []
    md.append(f"# Phase 7.5: {lang_name} Mobile STT Candidate Audit\n\n")
    ...
    md.append(f"In Phase 7.5, we audited open-source {lang_name} STT architectures and identified **Vakyansh Wav2Vec2 {lang_name}** as the primary lightweight candidate.\n\n")
...
def main():
    # 1. Audit Malayalam
    res_ml = audit_language(
        lang_name="Malayalam",
        model_id="Harveenchadha/vakyansh-wav2vec2-malayalam-mlm-8",
        manifest_rel="data/ml_in/manifest.json",
        fp32_onnx_name="vakyansh_malayalam_base.onnx",
        int8_onnx_name="vakyansh_malayalam_base.int8.onnx",
        vocab_name="malayalam_vocab.json",
        audit_md_name="malayalam_model_audit.md",
        results_csv_name="malayalam_results.csv",
        whisper_baseline_wer=193.8,
        whisper_baseline_cer=147.2,
        whisper_baseline_lat=1295.4,
        whisper_baseline_rtf=0.117,
        whisper_baseline_fail_reason="Extreme Hallucination / English Loop",
    )

    # 2. Audit Tamil
    res_ta = audit_language(
        lang_name="Tamil",
        model_id="Harveenchadha/vakyansh-wav2vec2-tamil-tam-250",
        manifest_rel="data/ta_in/manifest.json",
        fp32_onnx_name="vakyansh_tamil_base.onnx",
        int8_onnx_name="vakyansh_tamil_base.int8.onnx",
        vocab_name="tamil_vocab.json",
        audit_md_name="tamil_model_audit.md",
        results_csv_name="tamil_results.csv",
        whisper_baseline_wer=101.4,
        whisper_baseline_cer=78.6,
        whisper_baseline_lat=1308.2,
        whisper_baseline_rtf=0.119,
        whisper_baseline_fail_reason="Repetition Loop / Missing Tokens",
    )

    print("\n" + "=" * 80)
    print("  Malayalam & Tamil Audits Completed Successfully!")
    print("=" * 80)
