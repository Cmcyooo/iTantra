...
class HindiVakyanshBenchmarkTest {

    companion object {
        private const val TAG = "HindiVakyanshBenchmark"
        private const val MODEL_PATH = "/data/local/tmp/vakyansh_hindi_base.int8.onnx"
        private const val EXPECTED_SHA256 = "8e24e70119b8559d6299f68ae935be9999b93c1ea63f9d5c2191f902419aa516"
        ...
        val VOCAB = arrayOf(
            ...
            "ष", "स", "ह", "़", "ा", "ि", "ी", "ु", "ू", "ृ", "ॅ", "े", "ै", "ॉ", "ो", "ौ", "्"
        )
    }
    ...
    private fun readWavFloat32(file: File): FloatArray {
        ...
        // Standard PCM 16-bit mono 16 kHz WAV header is 44 bytes
        val dataOffset = 44
    ...
    private fun decodeCtc(logits: Array<Array<FloatArray>>): String {
        ...
        // CTC collapse repeated and remove blank/pad (pad=1, <s>=0, </s>=2, <unk>=3)
    ...
    @Test
    fun benchmarkHindiVakyanshOnDevice() {
        val modelFile = File(MODEL_PATH)
        Log.i(TAG, "Starting Android Vakyansh Benchmark on device...")
        ...
        val session = if (modelFile.exists()) {
            ...
            Log.w(TAG, "Model file not found at $MODEL_PATH. Generating dummy benchmark session.")
            null
        }
        ...
        if (session != null) {
            val testDir = File("/data/local/tmp/hindi_test")
            ...
            if (wavFiles.isNotEmpty()) {
                ...
                // Generate representative audio lengths (5s, 10s, 15s)
                val durations = listOf(5.0f, 10.0f, 13.0f, 15.0f)
                for ((idx, d) in durations.withIndex()) {
                    val numSamples = (d * 16000).toInt()
                    val dummyAudio = FloatArray(numSamples) { (Math.sin(it.toDouble() * 0.05) * 0.1).toFloat() }
                    sampleAudioList.add(dummyAudio)
                    sampleNames.add("synthetic_${d}s.wav")
                }
            }
            ...
            Log.i(TAG, "--- ON-DEVICE BENCHMARK SUMMARY ---")
            ...
            // 2. Stability Test: 10 repeated inferences on sample 0
        ...
        // Write output json to app internal storage and logcat
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        val outJsonFile = File(context.filesDir, "hindi_android_results.json")
